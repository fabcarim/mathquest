/* MathQuest – app.js v18 (overlay gestito SOLO via JS/inline style) */
...
function load(){
  ...
  try{
    ...
    Object.assign(state, {
      ...
      completions:s.completions||[], topicFilter:s.topicFilter||null,
      history:s.history||[]
    });
...
function setScreen(name){
  ...
  if(name==='play'){ if(!state.question) renderQuestion(); }
}
...
function renderStats(){
  ...
  // Aggiorna anche i campi dell’overlay (ma NON lo mostra)
  updateOverlayFields(corToday, state.dailyGoal, accAll);
}

/* ==== Domande (subset essenziale) ==== */
...
const Builders = {
  "Calcolo interi": D => {
    ...
    return makeQ('Calcolo interi','input', stem, 'Operazione: '+meta, ans, null, ["Esegui l'operazione"], String(ans));
  },
  "Percentuali": D => {
    const p=randChoice([10,12.5,20,25,30,40,50]); const x=randInt(16*D,80*D); const a=dec(x*p/100,2);
    return makeQ('Percentuali','input','Percentuale di una quantità','Calcola il '+p+'% di '+x+'. Arrotonda a 2 decimali.',a,null,['p/100 · valore'],String(a));
  }
};
function makeQuestion(){
  const D=state.difficulty;
  const keys=Object.keys(Builders);
  const key = state.topicFilter && Builders[state.topicFilter] ? state.topicFilter : randChoice(keys);
  setAccent(key);
...
function maybeCompleteGoal(){
  ...
  if(corToday >= state.dailyGoal && !state.goalShownToday){
    ...
    // Mostra overlay
    let n=5; $('countdown').textContent=n; showOverlay();
    const int=setInterval(()=>{ n--; $('countdown').textContent=n; if(n<=0){ clearInterval(int); hideOverlay(); setScreen('home'); } },1000);
    $('btnHomeNow').onclick=()=>{ hideOverlay(); clearInterval(int); setScreen('home'); };
  }
}
function confirmAnswer(){
  ...
  if(ok){
    ...
    state.streak=0;
    $('feedback').innerHTML="<div class='bad'><b>Non è corretto.</b></div>";
    setTimeout(autoAdjustDifficulty,0);
  }

  state.history.push({ topic: state.question.topic, type: state.question.type, stem: state.question.stem, meta: state.question.meta,
    given: state.input, answer: state.question.answer, ok, ts: Date.now(), diff: state.difficulty });

  state.answeredThis=true;
  const nx=$('btnNext'); if(nx) nx.disabled=false;
  renderStats(); save();
}
